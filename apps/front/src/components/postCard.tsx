...
const PostCard = ({
    ...
    return (
        <div className="bg-white rounded-lg shadow-md overflow-hidden flex flex-col">
            ...
            <div className="p-6 flex-grow bg-sky-100 flex flex-col">
                <h3 className="text-lg font-bold mt-4 break-words text-center text-gray-600">{title}</h3>
                <p className="mt-2 text-gray-500 text-sm">{dateText}</p>
                <p className="mt-4 text-gray-700 break-words ">{content?.slice(0, 100)}...</p>
                <Link className="text-indigo-600 hover:underline mt-auto block" href={`/blog/${slug}/${id}`}>Read more</Link>
            </div>
        </div>
    );
};
